...
<<<<<<< HEAD
		...
		glm::vec4 adjusted = lookAtMatrix * glm::vec4(teapots[i]->position,1);
		adjusted = clipPlaneMatrix * adjusted;
		bool check = adjusted.x < adjusted.w && adjusted.x > -adjusted.w &&
               adjusted.y < adjusted.w && adjusted.y > -adjusted.w &&
               0 < adjusted.z && adjusted.z < adjusted.w;
		if (check == true)
			teapots[i]->visible = true;	
		else
